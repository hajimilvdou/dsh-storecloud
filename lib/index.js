 ...
 *   serverUrl   商店服务器根地址（默认 https://blog.1qwq1.top —— 官方源，可用 ?server= 在页面上临时切换）
...
function apply(ctx, config) {
  ...
  async function httpHandler(req, res) {
    ...
    const localHost = host.indexOf('127.0.0.1:3080') === 0 || host.indexOf('localhost:3080') === 0
    const localOrigin = origin.indexOf('http://127.0.0.1:3080') === 0 || origin.indexOf('http://localhost:3080') === 0
    const allowed = site === 'same-origin' || (!site && !origin && localHost)
    if (!allowed && !(site === 'same-site' && localOrigin)) {
  ...
  async function rpcHandler(req, res) {
    ...
    if (op === '/install' || op === '/update') {
      ...
      const s3 = safeSpec(body.install || '')
      if (!s3) result = { ok: false, message: '客户端更新地址缺失' }
      else result = await runCli(['add', s3])
...
module.exports = { apply, name: 'dsh-storecloud' }
